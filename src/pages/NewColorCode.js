    ...
    function loop() {
      ...
      for (let i = 0; i < waves.length; i++) {
        ...
        for (let c = 0; c < waves[i].drips.length; c++) {
          waves[i].drips[c] += waves[i].settings.minSpeed + (noise(i * dripsAmount + c, currentTime) + 1) * waves[i].settings.maxSpeed;
        ...
        const hue = waves[i].settings.minHue + (currentTime % 1) * (waves[i].settings.maxHue - waves[i].settings.minHue);
        const saturation = waves[i].settings.minSaturation + (currentTime % 1) * (waves[i].settings.maxSaturation - waves[i].settings.minSaturation);
        const lightness = waves[i].settings.minLightness + (currentTime % 1) * (waves[i].settings.maxLightness - waves[i].settings.minLightness);
  ...
  const midAnimation = () => {
    ...
    function loop() {
      ...
      for (let i = 0; i < waves.length; i++) {
        ...
        context.drawImage(oldSceneryRef.current, 0, 0, 1568, 1120);
  ...
  return (
    <div id="HiddenColor">
      ...
      <div>
        ...
        <div
          id={newColorBucket[currentBucketIndex1].style}
          ref={bucketColorTextRef}
          style={{
            opacity: 0,
          }}
        >
